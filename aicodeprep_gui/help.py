...
Window opens up for aicodeprep-gui. All of your code files are already selected (checkmark on the files) which means, those files will be included when it generates the big block to paste.

what it does, is simply add every one of the checked files one after the other in a single block you can paste. It adds stuff like path/to/file.xyz and then whatever you type in the prompt box, gets added at the end (and if you choose, the beginning too - i'll explain why later but you can just not care and not worry about it honestly, just trust that it makes the AI respond better when it types it twice)

so when you click Process Selected button, it puts all those files one after the other and your prompt onto the clipboard, then you just paste it on an AI chat tab like ChatGPT, Gemini, Grok, Deepseek chat etc. 
...
My favorite button is the Cline Prompt. You can tell the AI to make something or modify/fix your project and then click that button to add a note telling the AI to make its answer a giant Cline prompt. This is the only time you have to copy and paste (just once though) it beats having to do it 146 times.
