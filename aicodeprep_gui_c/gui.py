...
class FileSelectionGUI(QtWidgets.QMainWindow):
    def __init__(self, files):
        ...
        self.text_label = QtWidgets.QLabel(""); self.text_label.setWordWrap(True); main_layout.addWidget(self.text_label)
        self.preset_strip = QtWidgets.QHBoxLayout(); self.preset_strip.addWidget(QtWidgets.QLabel("Presets:")); self.preset_strip.addStretch()
        add_preset_btn = QtWidgets.QPushButton("✚"); add_preset_btn.setFixedSize(24, 24); add_preset_btn.setToolTip("New Preset…"); add_preset_btn.clicked.connect(self.add_new_preset_dialog)
        ...
        for abs_path, rel_path, is_checked in files:
            ...
            for part in parts[:-1]:
                ...
                if path_so_far in self.path_to_item:
                    ...
                    QtWidgets.QTreeWidgetItem(new_parent, ["Loading..."])
            ...
            if os.path.isdir(abs_path):
                ...
                # LAZY LOADING: Add a placeholder so it's expandable
            ...
            try:
                for name in sorted(os.listdir(dir_path)):
                    ...
                    if os.path.isdir(abs_path):
                        ...
                        QtWidgets.QTreeWidgetItem(new_item, ["Loading..."]) # Recursive placeholder
                    else: # File
            ...
            try:
                ...
                def apply_to_children(parent_item, state):
                    for i in range(parent_item.childCount()):
                        ...
                        # Don't propagate to lazy-loading placeholders
    ...
    def _load_global_presets(self):
        try:
            for label, text in global_preset_manager.get_all_presets():
                self._add_preset_button(label, text, from_global=True)
        except Exception as e: logging.error(f"Failed to load global presets: {e}")
    ...
    def _delete_preset(self, label, button, from_global):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            self.preset_strip.removeWidget(button); button.deleteLater(); logging.info(f"Deleted preset: {label}")
